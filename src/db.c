...
// definindo o struct que ira receber os comandos
typedef struct {
  char* buffer;
...
// Representacao de uma pagina na memoria
typedef struct {
  int file_descriptor;
...
void* get_page(Pager* pager, uint32_t page_num) {
  ...
  if (pager->pages[page_num] == NULL) {
     // nao encontrou no cache. Aloca memoria e faz a leitura do arquivo
     void* page = malloc(PAGE_SIZE);
     uint32_t num_pages = pager->file_length / PAGE_SIZE;

     // salva uma pagina parcial no fim do arquivo
...
void leaf_node_insert(Cursor* cursor, uint32_t key, Row* value){
  ...
  if(cursor->cell_num < num_cells){
    ...
    for(uint32_t i = num_cells; i > cursor->cell_num; i--){
      memcpy(leaf_node_cell(node, i), leaf_node_cell(node, i - 1), LEAF_NODE_MAX_CELLS);
    }
  }
...
void internal_node_insert(Table* table, uint32_t parent_page_num,
  ...
  /**
   * Adiciona um novo par de filho/chave ao pai que corresponde ao filho
  */
  ...
  *internal_node_num_keys(parent) = original_num_keys + 1;
  ...
  if (original_num_keys >= INTERNAL_NODE_MAX_CELLS) {
    ...
    printf("Falta implementar a divisao de nodes internos\n");
    exit(EXIT_FAILURE);
  }
...
void pager_flush(Pager* pager, uint32_t page_num){
  if(pager->pages[page_num] == NULL){
    printf("Tentativa de liberar uma pagina null.\n");
    exit(EXIT_FAILURE);
  }
...
MetaCommandResult do_meta_command(InputBuffer* input_buffer, Table* table) {
  if (strcmp(input_buffer->buffer, ".exit") == 0) { // COMANDO PARA SAIR DO PROGRAMA
    db_close(table);
    exit(EXIT_SUCCESS);
  } else if(strcmp(input_buffer->buffer, ".btree") == 0){ // COMANDO PARA VISUALIZAR METADADOS
    printf("Tree:\n");
    print_tree(table->pager, 0, 0);
    return META_COMMAND_SUCCESS;
  } else if(strcmp(input_buffer->buffer, ".constants") == 0){ // COMANDO PARA VISUALIZAR METADADOS
    printf("Constantes:\n");
    print_constants();
    return META_COMMAND_SUCCESS;
  }
...
int main(int argc, char* argv[]) {
    ...
    if(argc < 2){
      printf("Necessario informar o nome do banco de dados.\n",0);
      exit(EXIT_FAILURE);
    }
    ...
    while (true) {
        ...
        switch (prepare_statement(input_buffer, &statement)) {
            ...
            case (PREPARE_STRING_TOO_LONG):
                printf("String ultrapassa o tamanho maximo para o campo.\n");
                continue;
